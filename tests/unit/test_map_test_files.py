...
""")
    ...
    def test_one_per_tc_strategy(self, tmp_path):
        ...
        self._create_tc_file(tc_dir, "TC-E2E-001")
        self._create_tc_file(tc_dir, "TC-E2E-002")

        result = map_test_files(str(tmp_path), ["TC-E2E-001", "TC-E2E-002"], "one-per-tc", "tests")
        ...
        assert file1["file_path"] == "tests/test_tc_e2e_001.py"
        assert file1["test_cases"] == ["TC-E2E-001"]
        assert file1["function_names"] == ["test_tc_e2e_001"]

        # Check second file
        file2 = data["file_mapping"][1]
        assert file2["file_path"] == "tests/test_tc_e2e_002.py"
        assert file2["test_cases"] == ["TC-E2E-002"]
        assert file2["function_names"] == ["test_tc_e2e_002"]
    ...
    def test_by_category_strategy(self, tmp_path):
        ...
        self._create_tc_file(tc_dir, "TC-NEG-001", title="Create notebook")
        self._create_tc_file(tc_dir, "TC-NEG-002", title="Delete notebook")
        self._create_tc_file(tc_dir, "TC-E2E-001", title="Full workflow")

        result = map_test_files(
            str(tmp_path), ["TC-NEG-001", "TC-NEG-002", "TC-E2E-001"], "by-category", "tests", feature_name="notebooks"
        )
        data = json.loads(result)

        # Should have 2 files (NEG, E2E)
        assert len(data["file_mapping"]) == 2

        # Find NEG file
        neg_file = next(f for f in data["file_mapping"] if "neg" in f["file_path"])
        assert neg_file["file_path"] == "tests/test_neg_notebooks.py"
        assert len(neg_file["test_cases"]) == 2
        assert "TC-NEG-001" in neg_file["test_cases"]
        assert "TC-NEG-002" in neg_file["test_cases"]
        assert "test_create_notebook" in neg_file["function_names"]
        assert "test_delete_notebook" in neg_file["function_names"]
    ...
    def test_by_category_with_subdirs_strategy(self, tmp_path):
        ...
        self._create_tc_file(tc_dir, "TC-NEG-001", title="Test API")
        self._create_tc_file(tc_dir, "TC-NFR-001", title="Test Unit")

        result = map_test_files(
            str(tmp_path), ["TC-NEG-001", "TC-NFR-001"], "by-category-with-subdirs", "tests", feature_name="notebooks"
        )
        data = json.loads(result)
    ...
    def test_function_name_generation(self, tmp_path):
        ...
        self._create_tc_file(tc_dir, "TC-NEG-001", title="Create API with special-chars & spaces!")

        result = map_test_files(str(tmp_path), ["TC-NEG-001"], "by-category", "tests", feature_name="feature")
        data = json.loads(result)
