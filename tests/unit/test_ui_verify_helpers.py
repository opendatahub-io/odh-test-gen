...
_SKILL_SCRIPTS = Path(__file__).resolve().parent.parent.parent / "skills" / "test-plan-ui-verify" / "scripts"
...
class TestUpdateLog:
    ...
    def _run(self, monkeypatch, tmp_path, tc_id, what, expected, result, detail, replace=False):
    ...
    def test_replace_removes_previous_entry_for_same_what(self, monkeypatch, tmp_path):
        ...
        log = self._run(monkeypatch, tmp_path, "TC-001", "filter visible", "ok", "PASS", "found", replace=True)
    ...
    def test_replace_preserves_other_assertions(self, monkeypatch, tmp_path):
        ...
        log = self._run(monkeypatch, tmp_path, "TC-001", "er2", "ok", "PASS", "now found", replace=True)
        assertions = log["TC-001"]["assertions"]
    ...
    def test_incomplete_verdict_not_overridden_by_pass(self, monkeypatch, tmp_path):
        ...
        log_path.write_text(
            json.dumps(
                {"TC-001": {"title": "TC-001", "verdict": "INCOMPLETE", "assertions": [], "blocked_reason": "crashed"}}
            )
        )
