...
| **Actionability** | Add concrete versions, test data formats with examples, test user roles with specific permissions and resources. Replace vague environment descriptions with actionable specifications. Retain genuinely unknown details only with an explicit resolution path: `TBD — Resolution: {concrete action} from/with/by/before/after/using {named source or timing}`; do not guess that path. | 3.1, 3.2, 3.3, 3.4 |
